#use "metro.ml" ;; 
(* r1 階乗を求め、 facの実行を始めた際にその引数を表示し、値を返す際に結果を表示する *)
let rec fac n =
  let rec fac1 n1 =
    if n1=0 then 1 else n1*fac1(n1-1)
  in  
  print_string "fac ";
  print_int n;
...
(* get_ekikan_kyori2 : string * string -> string * string ->  (string * string, ((string * string) * float) list) Tree.t -> float *)
...
let saitanro kiten_eki shuten_eki ekimei_l ekikan_l =
  let dijkstra kiten_ekimei shuten_ekimei ekimei_lst ekikan_lst =
    ...
    in (let rec pick_eki_t pair lst = match lst with
     ...
     | first :: rest -> (match first with
              | {namae = namae2_v; saitan_kyori = saitan_kyori3_v; temae_list = temae_list3_v} ->
                (if namae2_v = pair then first else pick_eki_t pair rest))
                            in pick_eki_t shuten_pair saitan_lst))))))
  in let dijkstra_result = dijkstra kiten_eki shuten_eki ekimei_l ekikan_l
  in let kyori = match dijkstra_result with
        {namae = namae_v; saitan_kyori = saitan_kyori_v; temae_list = temae_list_v} -> saitan_kyori_v
  in let saitan_keiro = match dijkstra_result with
        {namae = namae_v; saitan_kyori = saitan_kyori_v; temae_list = temae_list_v} ->
        let rec keiro_retu tem_lst = match tem_lst with
            [] -> ""
          | first :: rest -> match first with
              (eki,ken) -> match rest with
                [] -> eki
              | first1 :: rest1 -> eki^","^(keiro_retu rest)
        in keiro_retu temae_list_v
  in (print_string (kiten_eki^"と"^shuten_eki^"の最短距離は");
      print_float kyori;
      print_string ("kmです。最短経路は、"^saitan_keiro^"となっている。"))
